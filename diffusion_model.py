...
def get_timestep_embedding_adm(timesteps, num_channels, max_positions=10000, endpoint=False):
#     print("timesteps", timesteps.size())
    # Timestep embedding type: 'positional' for EDM, DDPM++, 'fourier' for NCSN++.
...
#     emb = timesteps.ger(freqs.to(timesteps.dtype))
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        if config.model.c_noise is not None:
            c_noise = config.model.c_noise
        else:
            c_noise = False
    ...
    def forward(self, x, t, return_intermediates = False):
...
#         x = x.float()
        ...
        intermediate_features = all_intermediate_features[4]
        all_return_features = []
        feat = torch.nn.functional.interpolate(intermediate_features, (self.config.data.image_size, self.config.data.image_size), mode='bilinear')
        all_return_features.append(feat)
        return_features = torch.cat(all_return_features, dim=1)
        del all_intermediate_features, intermediate_features, feat, all_return_features
