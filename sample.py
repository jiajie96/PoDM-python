"""Sampling / generation from a trained PoDM checkpoint."""
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Sample from trained PoDM")
    ...
    parser.add_argument("--sample_type", type=str, default="deterministic", choices=["deterministic", "stochastic"])
...
def main():
    ...
    with torch.no_grad():
        for r in range(n_rounds):
            ...
            if args.sample_type == "stochastic":
                x = edm_sampler(
                    x, args.timesteps, model,
                    sigma_min=sigma_min, sigma_max=sigma_max, rho=args.rho,
                    S_churn=40, S_min=sigma_min / 2 * 5, S_max=sigma_max / 8 * 5, S_noise=1.003,
                )
            else:
                x = edm_sampler(
                    x, args.timesteps, model,
    ...
    print(f"Generated {img_id} images in {elapsed:.2f}s -> {args.output_dir}")

    # Visualise a grid
    plt.figure(figsize=(40, 8))
    for i in range(min(20, img_id)):
        plt.subplot(2, 10, i + 1)
        img = x[i].permute((1, 2, 0)).numpy() if i < len(x) else np.zeros((config.data.image_size, config.data.image_size, 1))
        plt.imshow(np.asarray(img), cmap="gray")
        plt.axis("off")
    plt.savefig(os.path.join(args.output_dir, f"grid_{args.sample_type}_{args.timesteps}.png"))
    plt.close()
