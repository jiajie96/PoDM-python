...
ystick_labels = [0.02, 0.5 , 1, 5, 10, 20, 50]
...
key_points = [sigma_max-2, sigma_start+0.8, sigma_end+0.2, sigma_min+0.2, 2]
key_point_names = [r'$\sigma_{max}$', r'$\sigma_{start}$',r'$\sigma_{end}$',r'$\sigma_{min}$', 'feasibility-relevant noise scales']

for loc, point, txt in zip(key_point_locs[:-1], key_points[:-1], key_point_names[:-1]):
    ax.annotate(txt, (loc, point ** (1/y_scale)), fontsize=12)

ax.annotate(key_point_names[-1], (key_point_locs[-1], key_points[-1] ** (1/y_scale)), fontsize=10)

ax.hlines(sigma_end**(1/y_scale),colors='r', xmin=-1, xmax= 100, linestyles='dashed',alpha=0.5)
ax.hlines(sigma_start**(1/y_scale),colors='r',xmin=-1, xmax= 100,linestyles='dashed',alpha=0.5)
...
num_bins = 1000
ax_histy.hist(sigmas_training_hist, num_bins, density=True, color='orange', alpha=0.5, orientation='horizontal')
